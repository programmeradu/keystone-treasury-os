...
- Core product modules: CommandBar / natural-language treasury interface, Foresight Simulator (what-if engine), Solana Atlas (MEV/rug-pull/DCA), dreyv Studio (browser IDE + AI Architect), Marketplace (80/20 revenue split, Anchor devnet contract), Simulation Firewall, Persistent Knowledge Memory
...
- CommandBar (NL treasury interface): single 2500+ line monolithic `/api/command` route using `useChat` from `@ai-sdk/react` with streaming; primary model Groq llama-3.3-70b, fallback Cloudflare Workers AI; `DreyvAgentInput` component expects JSON responses but the API returns a stream (out of sync)
