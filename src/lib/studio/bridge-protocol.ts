...
// ─── Virtual SDK Module (serialized into iframe) ────────────────────
// This is the @keystone-os/sdk module that Mini-Apps import.
// It wraps keystoneBridge calls into React hooks.

export const VIRTUAL_SDK_MODULE = `
  ...
  export const useTurnkey = () => ({
    getPublicKey: async () => {
      return await keystoneBridge.call('turnkey.getPublicKey');
    },
    signTransaction: async (tx, description) => {
      return await keystoneBridge.call('turnkey.signTransaction', {
        transaction: tx,
  ...
  export function useFetch(url, options = {}) {
    ...
    const fetchData = useCallback(async () => {
      ...
      setError(null);
      try {
        const result = await keystoneBridge.call('proxy.fetch', {
          url,
  ...
  export const AppEventBus = {
    emit: (type, payload) => {
      keystoneBridge.notify('event.emit', { type, payload });
      console.log('[EventBus]', type, payload);
    },
  };

  // ─── Backward Compat (old import path) ───────────────────
  export default { useVault, useTurnkey, useFetch, AppEventBus };
`;
