...
const useCases = [
  {
    ...
    visual: (
      <svg viewBox="0 0 48 16" className="w-12 h-4">
        {[{w:14,c:"#a78bfa"},{w:10,c:"#60a5fa"},{w:8,c:"#9333ea"},{w:16,c:"#fbbf24"}].reduce<{el:React.ReactNode[],x:number}>((a,s,i) => {
  ...
  },
  {
    ...
    visual: (
      <svg viewBox="0 0 48 16" className="w-12 h-4">
        {[{x:6,c:"#7c3aed"},{x:18,c:"#60a5fa"},{x:30,c:"#a78bfa"}].map((d,i) => (
          <g key={i}><circle cx={d.x} cy="8" r="3" fill={d.c} opacity="0.2" /><circle cx={d.x} cy="8" r="1" fill={d.c} opacity="0.4" /></g>
        ))}
        <line x1="9" y1="8" x2="15" y2="8" stroke="#94a3b8" strokeWidth="0.5" opacity="0.35" />
        <line x1="21" y1="8" x2="27" y2="8" stroke="#94a3b8" strokeWidth="0.5" opacity="0.35" />
      </svg>
    ),
  },
  {
    ...
    visual: (
      <svg viewBox="0 0 48 16" className="w-12 h-4">
        {[0, 10, 20, 30, 40].map((x, i) => (
          <rect key={i} x={x} y="5" width="6" height="6" rx="1" fill="#9333ea" opacity={0.1 + i * 0.06} />
        ))}
      </svg>
    ),
  },
  ...
  },
  {
    ...
    visual: (
      <svg viewBox="0 0 48 16" className="w-12 h-4">
        ...
        <path d="M 24 8 L 36 4 L 48 2" fill="none" stroke="#7c3aed" strokeWidth="0.8" opacity="0.28" strokeDasharray="2 2" />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-0">
          {useCases.map((uc, i) => (
            <motion.div
              ...
              className="group border-t lg:[&:nth-child(-n+3)]:border-t sm:[&:nth-child(-n+2)]:border-t border-violet-200/35 p-8 hover:bg-white/[0.015] transition-colors duration-300"
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-mono text-violet-600/85">
