...
export function ExecutionDashboard({
  ...
  // ⚡ Bolt: Consolidated multiple O(N) status filters into a single O(N) reduce pass
  // Performance impact: Reduces array iterations from 3 to 1 for the dashboard summary, critical for auto-refresh
      ...
      {activeExecutions.length > 0 && (
        <div className="border-t border-slate-700 pt-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <StatCard
              label="Running"
              value={executionStats.running.toString()}
              color="cyan"
            />
            <StatCard
              label="Pending"
              value={executionStats.pending.toString()}
              color="gray"
            />
            <StatCard
              label="Approval"
              value={executionStats.approval.toString()}
              color="yellow"
            />
          </div>
        </div>
