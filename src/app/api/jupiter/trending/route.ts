  ...
  try {
    ...
    const sources: Array<() => Promise<any>> = [
      ...
      async () => {
        ...
        const trending = ["WIF", "JTO", "MNGO", "MOBILE", "RENDER", "DRIFT", "TNSR", "MNDE"];
    ...
    const normalized = items.map((t: any) => ({
      ...
      .filter((t: any) => t.mint && t.symbol && !["SOL", "MSOL", "USDC", "USDT", "BONK", "JUP", "PYTH", "RAY", "ORCA"].includes(t.symbol.toUpperCase()));
    ...
    const fallback = [
      ...
      { mint: "DriFtupJYLTosbwoN8koMbEYSx54aFAVLddWsbksjwg7", symbol: "DRIFT", name: "Drift Protocol", icon: "https://metadata.jup.ag/token/DriFtupJYLTosbwoN8koMbEYSx54aFAVLddWsbksjwg7/logo" },
      { mint: "TNSRxcUxoT9xBG3de7PiJyTDYu7kskLqcpddxnEJAS6", symbol: "TNSR", name: "Tensor", icon: "https://metadata.jup.ag/token/TNSRxcUxoT9xBG3de7PiJyTDYu7kskLqcpddxnEJAS6/logo" },
      { mint: "7dHbWXmci3dT8UFYWYZweBLXukXe1uT238VHbQZjY67Q", symbol: "stSOL", name: "Lido Staked SOL", icon: "https://crystalballs.mypinata.cloud/ipfs/QmTbFZ8hRq1NpFm3nYpdj4pPdZs3kY8X9jg3QrA3b3Rr3s" },
    ];
    return NextResponse.json({ items: fallback.slice(0, Math.max(1, Math.min(10, Number(new URL(req.url).searchParams.get("limit") || 8)))) , warning: e?.message || String(e) }, { status: 200 });
  }
}