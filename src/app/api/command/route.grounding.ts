export type EvidenceClass = "conversational" | "tool_required" | "db_required" | "navigation_required";
export type GroundingGateResult = { allowed: boolean; blockedReason?: string };

export function classifyEvidenceNeed(userText: string): EvidenceClass {
  const isSimpleConversation = (t: string) => /^(hi|hello|hey|gm|gn)/.test(t);
