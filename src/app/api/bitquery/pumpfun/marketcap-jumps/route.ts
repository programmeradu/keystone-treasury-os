...
// Default subscription: stream newly-created Pump.fun tokens metadata via TokenSupplyUpdates filter
...
export async function GET(req: NextRequest) {
  ...
  // Prefer explicit ?token= override, else fall back to env
  const token =
    ...
    "";
  if (!token) {
    const body = `data: ${JSON.stringify({ type: "error", error: "Missing Bitquery token. Set BITQUERY_BEARER or BITQUERY_API_KEY in env." })}\n\n` +
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      ws.on("open", () => {
        ...
        ws.send(connInit());
        // protocol keepalive from our side
        wsPing = setInterval(() => { try { ws.send(ping()); } catch {} }, 20000);
        // Send subscription (customizable)
        ...
        try {
          ...
          switch (msg.type) {
            case "connection_ack":
              ...
              break;
            case "next":
              // Stream Bitquery payloads directly (normalized minimally)
            ...
            case "ping":
              ...
              break;
            default:
              // Pass-through for unknown frames for debugging
      ...
      // Client aborted the HTTP request
      // @ts-ignore
