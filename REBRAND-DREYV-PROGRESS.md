...
- [x] Add this progress file (`REBRAND-DREYV-PROGRESS.md`).
- [x] Add `public/llms.txt` for LLM/GEO discovery.
- [x] Add FAQ section on home + **FAQPage** JSON-LD (aligned copy).
- [x] Add **What is dreyv?** section on home (hero + dedicated blurb).
- [x] Align root metadata / manifest naming with lowercase **dreyv** where brand-facing.
- [x] Add `--dreyv-*` / `--color-dreyv-*` theme aliases in `globals.css`.

## Phase 2 — Landing & home components

- [x] Header/footer: logo label, aria-labels, nav (incl. FAQ anchor).
- [x] Hero, problem, pillars, demo, war-room, trust, CTA, command bar, ecosystem, use-cases, how-it-works: user-facing **Keystone** → **dreyv**; mock `keystone://` → `dreyv://` where decorative; accent classes moved to **`dreyv-*`** on these sections.
- [x] Deep dive: dashboard alt text; code sample still uses `@keystone/sdk` until Phase 5 npm scope.
- [x] Public marketing/legal routes: `/pricing`, `/auth`, `/guides/...`, `/legal/*`, `error.tsx`, `/mobile/signer` — product copy **dreyv**; cookie table still lists real cookie names (`keystone-*`).
- [x] Footer socials: **X @dreyvapp** (`https://x.com/dreyvapp`), **LinkedIn** (`https://www.linkedin.com/company/dreyv/`); JSON-LD **sameAs** updated.
...
- [x] `/app` routes + in-app components: user-visible **Keystone** → **dreyv** (headers, marketplace, studio defaults, onboarding, command bar, exports, foresight, billing, etc.). Hooks like `useKeystoneAuth` and runtime bridge identifiers unchanged.
- [x] Transactional email copy + `email-service` subjects/from default; React email layout exported as **`DreyvLayout`**.
- [x] Legal pages: product name **dreyv** in body copy; IP section attributes rights to **StaUniverse**; contact emails unchanged (`@keystone.so`) until mailboxes migrate.
- [x] Global `error.tsx` footer branding; **auth** page visible strings (session keys / hooks unchanged).
