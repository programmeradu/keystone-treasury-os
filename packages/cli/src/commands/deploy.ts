...
export async function runDeploy(options: DeployOptions): Promise<DeployResult> {
    ...
    // Deploy program
    try {
        // [SECURITY] Use arguments array with solana program deploy to prevent command injection
        ...
        if (!options.skipIdl) {
            ...
            if (fs.existsSync(idlPath)) {
                try {
                    // [SECURITY] Use arguments array with anchor idl init to prevent command injection
                    await execAsync(
                        "anchor",
