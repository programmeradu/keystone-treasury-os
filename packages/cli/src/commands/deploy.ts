...
export async function runDeploy(options: DeployOptions): Promise<DeployResult> {
    ...
    // Set cluster
    try {
        await execFileAsync("solana", ["config", "set", "--url", getClusterUrl(cluster)]);
    } catch (err: any) {
    ...
    try {
        const { stdout } = await execFileAsync("solana", ["balance"]);
    ...
    // Deploy program
    try {
        const deployArgs = ["program", "deploy", soPath];
        ...
        const { stdout, stderr } = await execFileAsync("solana", deployArgs, {
            cwd: targetDir,
        ...
        if (!options.skipIdl) {
            ...
            if (fs.existsSync(idlPath)) {
                try {
                    await execFileAsync(
                        "anchor",
                        ["idl", "init", "--filepath", idlPath, programId],
